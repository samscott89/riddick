import { parseRustCode } from '../../src/parser'
...
async function queue(batch: MessageBatch<QueueMessage>, env: Env): Promise<void> {
...
async function processParseStage(
  ...
  try {
    ...
    if (parseResult.success && parseResult.crateInfo) {
      ...
      for (const item of parseResult.crateInfo.items) {
